...
class Model_CNN(Layer):
    ...
    def load_model(self, param_list):
        ...
        assert model_type == 'Model_CNN', 'Loaded model is not Model_CNN.'

        # Re-init then overwrite parameters
        self.__init__()
        trainable_layers = [layer for layer in self.layers if layer.optimizable]
        for i, layer in enumerate(trainable_layers):
            ...
            layer.weight_decay_lambda = param_list[i + 1]['lambda']
        
    def save_model(self, save_path):
        param_list = ['Model_CNN']
        for layer in self.layers:
            if layer.optimizable:
                param_list.append({
                    'W': layer.params['W'],
